...
import { IsDacenOperator, IsOperator } from "@/validation/validateGroupAkses";
...
const page = ({ headers, data, action, link }) => {
  ...
  const sortedData = data.slice().sort((a, b) => {
    ...
    if (a.dacen_status === "Ongoing" && b.dacen_status === "Ongoing") {
      ...
      return (
        new Date(a.dacen_deadline_project) - new Date(b.dacen_deadline_project)
      );
    }
      ...
      <table className="table">
        <thead>
          <tr className="border-b-2 bg-[#00A6B4]/[0.5] text-sm text-center uppercase">
            ...
            {(IsOperator() || IsDacenOperator()) && action && (
