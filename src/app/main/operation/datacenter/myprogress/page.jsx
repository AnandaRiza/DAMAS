'use client';
import Header from '@/components/operation/datacenter/header/header_my_project';
import Body from '@/components/operation/datacenter/body/operation_datacenter_my_project';
...
const page = () => {
  ...
  const getDataAllDacen = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_DAMAS_URL_SERVER}/dacenshow/userdomainprojects?start=${startIndex}&size=${perPage}&userdomain=${user.userdomain}`
      );
  ...
  return (
    ...
    <div className="flex-grow justify-center items-center min-h-screen bg-white rounded-xl px-3">
      ...
      {dataAllDacen && dataAllDacen.length === 0 ? (
        <NotFound />
      ) : (
        <div>
          {dataAllDacen && (!searchResult || searchInput === "") ? (
            <Body
              ...
              data={dataAllDacen}
              action={true}
              link={"/main/operation/datacenter/myprogress/"}
          ...
          {dataAllDacen && (
            <div className="w-full flex justify-end items-center gap-3">
              ...
              <button
                type="button"
                disabled={startIndex + perPage >= dataAllDacen[0].maxSize}
                onClick={() => {
                  setCurrentPage(currentPage + 1);
                  setStartIndex(startIndex + 20);
                }}
