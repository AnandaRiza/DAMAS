...
import Header from '@/components/operation/itmo/header/header_all_project';
import Body from '@/components/operation/itmo/body/operation_itmo_table';
  ...
  const getDataAllItmo = async () => {
    ...
    try {
        const response = await axios.get(
            `${process.env.NEXT_PUBLIC_DAMAS_URL_SERVER}/itmoshow?start=${startIndex}&size=${perPage}`
        );
    ...
    <div style={{ position: 'absolute', top: 30, right: 45 }}>
            ...
            {dataAllItmo &&  (
                <div className="w-full flex justify-end items-center gap-3 mt-2">
                    ...
                    <button
                        disabled={
                            startIndex + perPage >= dataAllItmo[0].maxSize
                        }
                        onClick={() => {
                            setCurrentPage(currentPage + 1);
                            setStartIndex(startIndex + 10);
