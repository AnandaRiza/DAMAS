"use client";
import Header from "@/components/operation/itsecurity/header/header_my_project";
import Body from "@/components/operation/itsecurity/body/operation_itsecurity_my_project";
...
const page = () => {
  ...
  const getDataAllItsecurity = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_DAMAS_URL_SERVER}/itsecurityshow/userdomainprojects?start=${startIndex}&size=${perPage}&userdomain=${user.userdomain}`
      );
  ...
  return (
    ...
    <div className="flex-grow justify-center items-center min-h-screen bg-white rounded-xl px-3">
      ...
      {dataAllItsecurity && dataAllItsecurity.length === 0 ? (
        <NotFound />
      ) : (
        <div>
          {dataAllItsecurity && (!searchResult || searchInput === "") ? (
            <Body
              ...
              action={true}
              link={"/main/operation/itsecurity/myprogress/"}
          ...
          {dataAllItsecurity && (
            <div className="w-full flex justify-end items-center gap-3">
              ...
              <button
                type="button"
                disabled={startIndex + perPage >= dataAllItsecurity[0].maxSize}
                onClick={() => {
                  setCurrentPage(currentPage + 1);
                  setStartIndex(startIndex + 20);
                }}
